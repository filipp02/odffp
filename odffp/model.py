...
class OdffpFit(OdfFit):
    ...
    def _export_dict_var(self, dict_var, fixed_peaks_num=None, peak_id=None, full_volume=False):
        ...
        if full_volume:
            ...
            # Disregard "empty" voxels, i.e. these with no signal
    ...
    def copy(self, mask=None):
        if mask is None:
            mask = np.ones(self._data.shape[:-1], dtype=bool)
        else:
            mask = mask.astype(bool)
        ...
        odf = np.zeros_like(self._odf)
        odf[mask] = self._odf[mask]

        peak_dirs = np.zeros_like(self._peak_dirs)
        peak_dirs[mask] = self._peak_dirs[mask]

        dict_idx = np.zeros_like(self._dict_idx)
        dict_idx[mask] = self._dict_idx[mask]

        return OdffpFit(data, self._dict, odf, peak_dirs, dict_idx)


    def merge(self, other_odffp_fit):

        # Take all non-empty voxels from other_odffp_fit
        mask = other_odffp_fit._dict_idx > 0
